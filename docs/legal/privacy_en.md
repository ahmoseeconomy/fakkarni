...
your phone's own speech-recognition service (Apple or Google). **On iPhone the
audio is sent to Apple's servers** (on-device Arabic recognition is not
guaranteed, so we do not require it); **on Android** we ask for on-device
recognition, and where the phone cannot do that for Arabic the audio goes to
Google's servers — under their terms (see §5). You can turn voice off in Settings or decline the microphone
...
| **System speech recognition** (Apple on iPhone, Google on Android) | **Your voice** while you answer with the «اتكلم» button — to turn it into text | Only when you press «اتكلم». **iPhone: the audio is sent to Apple's servers.** Android: we request **on-device** recognition; where the phone cannot do that for Arabic, the audio goes to Google's servers. We never receive or store the audio. [CONFIRM: which devices and OS versions recognise Arabic on-device — the company measures this on real phones] |
