...
class RecordsRepository {
  ...
  /// بيغيّر اسم السجل ودكتوره — والمتابعة هي اللي محتاجة ده فعلاً.
  ///
  /// اسم المتابعة بيتولد من الورقة أول ما تبدأ، والورقة ساعات ما بيكونش
  /// فيها اسم («من غير اسم دكتور»)، أو الاسم اتقرا غلط. من غير باب
  /// للتعديل، الراجل بيفضل قاعد قدّام متابعة مالهاش اسم يعرفها بيه —
  /// وهو اللي يعرف، مش إحنا. المسح مش الحل: ده بيرمي المراحل والمواعيد.
  ///
  /// الدكتور الفاضي بيتحفظ null، زي كل حقل تاني هنا — مفيش نص فاضي
  /// بيتخزّن كأنه قيمة.
  Future<void> rename(int id, {required String title, String? doctor}) {
    final t = title.trim();
    if (t.isEmpty) throw ArgumentError.value(title, 'title', 'العنوان فاضي');
    final d = doctor?.trim();
    return (_db.update(_db.records)..where((r) => r.id.equals(id))).write(
      RecordsCompanion(title: Value(t), doctor: Value(d == null || d.isEmpty ? null : d)),
    );
  }
