...
- **المقدمة**: `VoiceIntroScreen` قبل «نتعرّف عليك» مرة واحدة (`voice.introDone`)،
...
- **«🔊 ساعدني»** (`HelpButton(id)` / `HelpRow`) — **بيظهر بس لما الصوت شغّال**؛
  ...
  **ما اتوصّلوش لمكان**: `help_family` (مفيش شاشة «ضيف عيلة» غير شاشة الكود
  اللي عليها `help_invite_code`)، و`gen_saved` / `gen_no_medical` /
  `gen_try_hands` / `gen_goodbye` — مسجّلين ومستنيين فعل يستاهلهم.
- **خطوات البداية بتتكلم لوحدها** (٢٦ سبتمبر ٢٠٢٦): `onb_*` (١١ جملة، من
  ...
  يختار أو يحرّك بكرة أو يسيب الصفحة. `onb_entry` على شاشة البداية (أول فتحة
  خالص المقدمة لسه ما جتش، فهي ساكتة — بتتكلم لو رجع لها)؛ «نتعرّف عليك» بقت
