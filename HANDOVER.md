...
| B7 | Payments: built behind one seam, not sellable yet | **Model (owner, 24 Sep 2026): ONE family subscription per patient covers the patient and up to 5 followers/nurses — not «each person pays».** Built: `supabase/migrations/0025_family_subscription.sql` (table `family_subscriptions`, 14-day trial on patient creation, 30 days for existing patients, `private.family_subscription_active`, `due_escalations` gated through `private.follower_subscription_active(caregiver, patient)`, circle cap 5 via `circle_full` in `redeem_invite`) — **not yet run**; the Edge Function `supabase/functions/verify-purchase` (Apple App Store Server API + Google `androidpublisher`, credentials from Edge secrets only; returns `not_configured` when missing) — **never deployed or executed**; the paywall «اشتراك العيلة» (prices from the store via `in_app_purchase`, restore, «تذكير الدوا مجاني للأبد»); gates on scans, invites and PDF export beyond 3. **Safety rules are structural**: reminders, the ladder and the son's alert never read billing (guard test `billing_mirror_test`), and an unreachable verifier keeps the last known state. **Before selling, the company must:** (1) run `0025` and confirm `verify_migrations.sql` shows 25 rows `ok = true`; (2) create the subscription products `fakkarni_family_monthly` / `fakkarni_family_yearly` in App Store Connect (one subscription group) and Google Play Console (one base plan each), sign the Paid Apps Agreement, fill bank and tax forms; (3) set Edge secrets `APPLE_ISSUER_ID`, `APPLE_KEY_ID`, `APPLE_PRIVATE_KEY` (In-App Purchase key from App Store Connect), `APPLE_BUNDLE_ID`, `GOOGLE_SERVICE_ACCOUNT_JSON` (service account with «View financial data» on the Play app), `ANDROID_PACKAGE`, then `supabase functions deploy verify-purchase`; (4) test one sandbox purchase per store and watch a `family_subscriptions` row turn `active`; (5) confirm the free/family list in «Pricing» in `CLAUDE.md` (it is a default awaiting the owner's confirmation). Subscription **renewal and cancellation webhooks (App Store Server Notifications / Play RTDN) are not built** — status is refreshed only when the app verifies again; an expired row simply stops gating after `expires_at` + 3 days grace on the next verification. |
