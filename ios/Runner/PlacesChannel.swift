...
///   { id, kind: "pharmacy" | "doctor", name, lat, lon, phone }
///
/// * `MKLocalSearch` باستعلام بلغة طبيعية — واحد للصيدليات وواحد للدكاترة —
///   جوّه منطقة نصف قطرها الراديوس، وبعدين **فلترة بالمسافة** لأن MapKit
///   بيرجّع اللي حوالين المنطقة مش اللي جوّاها بس.
...
final class PlacesChannel {
  ...
  /// استعلامين بالتوازي، وبعدين دمج من غير تكرار (نفس المكان ممكن يرجع في
  /// الاتنين — عيادة اسمها فيه «صيدلية»).
  private static func nearby(lat: Double, lon: Double, radius: Double) async throws -> [[String: Any]] {
    ...
    let items = try await pharmacies + doctors
