...
values
  ...
  -- ✘ الجهاز كتب «اتنست» خلاص — القرار اتاخد، مش pending
...
-- ================================ ٢) الاختيار: واحد بالظبط، وبالسبب الصح
do $$
declare
  v_count integer;
  v_event uuid;
...
begin
  ...
  if v_count <> 1 then
    raise exception 'FAIL: المفروض صف واحد مستحق، جه %', v_count;
  end if;

  select dose_event_uuid, caregiver_id, patient_name, medication_name
    into v_event, v_care, v_name, v_med
    from private.due_escalations();

  if v_event <> 'e1000000-0000-0000-0000-000000000001' then
    raise exception 'FAIL: اتختار حدث غلط: %', v_event;
  end if;
...
begin
  for r in
    select * from (values
      ...
      ('e5000000-0000-0000-0000-000000000005'::uuid, 'اتكتبت «اتنست» خلاص'),
