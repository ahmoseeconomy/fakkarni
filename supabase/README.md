...
25. `migrations/0026_nurse_account.sql` — حساب الممرض: الكود بصلاحية
    التعديل، `redeem_invite(p_code, p_expect_role)` بترفض الباب الغلط من
    غير ما تحرق الكود، أعمدة الغرض والتعليمات ونوع التنبيه على
    `medications`، تغييرات معلّقة للورقة والميعاد، الكتابة من الدائرة مع
    الاشتراك بس، وباكت `patient-papers` الخاص (المالك يكتب، المالك
    وممرضينه يقروا). **لسه ما اتشغّلتش** — بعد 0025.
26. `migrations/0027_vitals.sql` — القياسات الحيوية: جدول `vitals`، المالك
    بيكتب، الدائرة (عيلة وممرض) بتقرا بـ`can_access_patient`. **لسه ما
    اتشغّلتش** — بعد 0026.
27. `migrations/0028_medication_stock.sql` — مخزون الأدوية: جدول
    `medication_stock` (المالك بيكتب، الدائرة بتقرا)، وطلب «علبة جديدة»
    (`restock`) من الممرض. **لسه ما اتشغّلتش** — بعد 0027.
28. `migrations/0029_med_photos.sql` — صور الأدوية للدائرة على نفس باكت
    `patient-papers` تحت `{patient}/med-photos/`: سياسات **جديدة** (قراية
    للدائرة كلها، الرسمية للمالك، والممرض بـ«يعدّل الأدوية» بيرفع تحت
    `pending/`) ونوع تغيير `photo`. سياسات 0026 للورق ما اتلمستش. **لسه ما
    اتشغّلتش ومستنية مراجعة** — بعد 0028.
29. `migrations/0030_patient_papers_limits.sql` — حدود الباكت: ١٠ ميجا و
    `image/jpeg` بس (اللي التطبيق بيرفعه بالظبط). فحصها كـpostgres بس.
    **لسه ما اتشغّلتش** — بعد 0029.
30. `migrations/0031_not_bought.sql` — «أدوية لسه ماتشترتش» للدائرة: عمود
    `medications.not_bought_at` ونوع تغيير `bought`. **لسه ما اتشغّلتش
    ومستنية مراجعة** — التطبيق لسه ما بيرفعش العمود ولا بيقراه.
31. `verify_migrations.sql` — **بيقرا بس** (SELECT واحد، مفيش DDL ولا
    كتابة): صف لكل ترحيل من 0001 لـ0031 بـ`expected`/`found`/`ok`/
